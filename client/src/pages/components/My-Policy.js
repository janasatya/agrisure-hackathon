...
export default function MyPolicy({ webApi ,setSelect}) {
  ...
  function UserInput({ data, webApi }) {
    ...
    return (
      <div>
        <h1 className="text-xl  text-center p-4 bg-slate-400"> First transaction {first}</h1>
        <h1 className="text-xl  text-center p-4 bg-slate-400"> Second transaction {second}</h1>
      </div>
    )
    }
    if(load==2){
      ...
      return (
        ...
        <h1 className="text-xl  text-center p-4 bg-slate-400">Wait for a minute......</h1>
        {
          tcprocess()
        }
        </div>
